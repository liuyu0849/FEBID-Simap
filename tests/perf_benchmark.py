...
def write_report(results, t_numba, t_numpy):
    ...
    L.append("")
    L.append("**1. 反应步：平坦路段不必派四名侦察兵。**  ")
    L.append("RK4 每前进一步要做四次试探性求值，好比每走一步都派四名侦察兵探路——"
             "在悬崖峭壁（强刚性、剧烈变化）这是必要的；但本体系的吸附/解吸时间常数"
             "远长于当前步长，路面平坦，两名侦察兵（低阶格式）就能达到同等落脚精度。"
             "目标：在保证 1% 一致性容忍度的前提下降低每步的求值次数，"
             "把逐点成本压掉约一半。该项须以一致性测试验证后方可采纳。")
    L.append("")
    L.append("**2. 扩散步：让它敢迈大步，并合并出发。**  ")
    L.append("显式扩散像每步只许热量传给紧邻，迈大步就“摔跤”，步长被死死限制；"
             "隐式 / ADI 允许一步把影响稳稳传到远处而不失稳——单步略贵，"
             "却能用大步长**减少总步数**，这也是解开 COG 体系死结的唯一正路。"
             "另外各物种目前各自“出发一趟车队”（一次并行启动），合并为一趟跑完"
             "所有物种可省去反复集结解散的固定开销。")
    L.append("")
